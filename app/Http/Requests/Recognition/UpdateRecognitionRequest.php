...
class UpdateRecognitionRequest extends FormRequest
{
    ...
    {
        return $this->user()->can('update', $this->route('recognition'));
    }
    ...
    {
        return [
            'category' => 'sometimes|required|string|in:' . implode(',', config('recognition.categories')),
            'total_mark' => 'sometimes|required|integer|min:' . config('recognition.marks.min') . '|max:' . config('recognition.marks.max'),
