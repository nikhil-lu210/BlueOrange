...
@section('page_title', __('Learning Topic Details'))
...
@section('page_name')
    <b class="text-uppercase">{{ __('Learning Topic Details') }}</b>
@endsection
...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item">
        <a href="{{ route('administration.learning_hub.my') }}">{{ __('My Learning Topics') }}</a>
    </li>
    <li class="breadcrumb-item active">{{ __('Learning Topic Details') }}</li>
@endsection
...
<div class="row justify-content-center">
                ...
                <div class="flex-grow-1 mt-4">
                    <div class="d-flex align-items-center justify-content-md-between justify-content-start mx-4 flex-md-row flex-column gap-4">
                        <div class="user-profile-info">
                            <h4 class="mb-0">{{ $learning_topic->title }}</h4>
                            <ul class="list-inline mb-0 d-flex align-items-center flex-wrap justify-content-sm-start justify-content-center gap-2">
                                <li class="list-inline-item d-flex gap-1" data-bs-toggle="tooltip" title="Creator" data-bs-placement="bottom">
                                    ...
                                    {{ $learning_topic->creator->alias_name }}
                                </li>
                                <li class="list-inline-item d-flex gap-1" data-bs-toggle="tooltip" title="Learning Topic Date & Time">
                                    <i class="ti ti-calendar"></i>
                                    {{ show_date_time($learning_topic->created_at) }}
                                </li>
                            </ul>
                            @if (!is_null($learning_topic->recipients))
                                <ul class="list-inline mb-0 mt-3 d-flex align-items-center flex-wrap justify-content-sm-start justify-content-center gap-1">
                                    @foreach ($learning_topic->recipients as $recipient)
                                        <li class="list-inline-item d-flex gap-1 badge bg-label-dark">
                                            {{ $recipient->alias_name }}
                                        </li>
                                    @endforeach
                                </ul>
                            @endif
                        ...
                        @can ('Learning Hub Update')
                            <a href="{{ route('administration.learning_hub.edit', ['learning_topic' => $learning_topic]) }}" class="btn btn-dark btn-icon rounded-pill" data-bs-toggle="tooltip" title="Edit Learning Topic">
    ...
    <div class="col-md-7">
        <div class="card mb-4">
            <div class="card-header header-elements">
                <h5 class="mb-0">Learning Topic Details</h5>

                <div class="card-header-elements ms-auto">
                    <button type="button" class="btn btn-primary btn-sm btn-icon rounded-pill" title="Seen & Read By" data-bs-toggle="modal" data-bs-target="#showLearningTopicReadersModal">
            ...
            <!-- Account -->
            <div class="card-body">
                {!! $learning_topic->description !!}
            </div>
        </div>

        <div class="card mb-4">
            <div class="card-header header-elements pt-3 pb-3">
                <h5 class="mb-0">Learning Topic Files</h5>
            </div>

            @if ($learning_topic->files->count() > 0)
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table table-bordered">
                            ...
                            <tbody>
                                @foreach ($learning_topic->files as $file)
                                    <tr>
                                        <td>
                                        ...
                                        <td class="text-center">
                                            @if ($learning_topic->creator_id == auth()->user()->id)
    ...
    {{-- Learning Topic Comments --}}
    <div class="col-md-5">
        <div class="card mb-4">
            <div class="card-header header-elements">
                <h5 class="mb-0">Learning Topic Comments</h5>
            ...
            <div class="card-body">
                <div class="row">
                    <div class="col-md-12">
                        <form action="{{ route('administration.learning_hub.comment.store', ['learning_topic' => $learning_topic]) }}" method="post">
                            @csrf
                            <div class="collapse" id="collapseExample">
                                <div class="row">
                                    <div class="col-md-12">
                                        <textarea class="form-control" name="comment" rows="2" placeholder="Ex: This learning topic is very helpful." required>{{ old('comment') }}</textarea>
                                        @error('comment')
                ...
                <div class="row">
                    <div class="col-md-12 comments">
                        <table class="table">
                            <tbody>
                                @foreach ($learning_topic->comments as $comment)
...
{{-- Read By At Modal --}}
<div class="modal fade" data-bs-backdrop="static" id="showLearningTopicReadersModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content p-3">
            <button type="button" class="btn-close btn-pinned" data-bs-dismiss="modal" aria-label="Close"></button>
            <div class="modal-body">
                <div class="text-center mb-4">
                    <h3 class="role-title mb-2">Learning Topic Readers</h3>
                    <p class="text-muted">Details of Learning Topic Readers</p>
                </div>
                ...
                <div class="row">
                    <div class="col-md-12">
                        <table class="table table-bordered">
                            ...
                            <tbody>
                                @foreach ($learning_topic->read_by_at as $reader)
                                    <tr>
                                        <td>{{ $reader->read_by->alias_name }}</td>
                                        <td>{{ show_date_time($reader->read_at) }}</td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
