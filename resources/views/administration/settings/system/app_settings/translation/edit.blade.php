...
@section('page_title', __('Edit Translation'))
...
@section('page_name')
    <b class="text-uppercase">{{ __('Edit Translation') }}</b>
@endsection
...
<div class="row">
    <div class="col-md-12">
        <div class="card mb-4">
            ...
            <div class="card-body">
                <form action="{{ route('administration.settings.system.app_setting.translation.update', ['translation' => $translation]) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="row">
                            ...
                            <select name="locale" id="locale" class="form-select @error('locale') is-invalid @enderror" required>
                                ...
                                @foreach($localeDetails as $code => $locale)
                                    @if($code !== 'en' && $locale['will_use'])
                                        <option value="{{ $code }}" {{ old('locale', $translation->locale) == $code ? 'selected' : '' }}>
                        ...
                        <div class="col-md-12 mb-3">
                            ...
                            <textarea 
                                ...
                                required>{{ old('translated_text', $translation->translated_text) }}</textarea>
                        ...
                        <div class="col-md-12 mb-3">
                            <div class="alert alert-info">
                                <strong>Created:</strong> {{ $translation->created_at->format('d M Y, h:i A') }}<br>
                                <strong>Last Updated:</strong> {{ $translation->updated_at->format('d M Y, h:i A') }}
                            </div>
                        </div>
                    </div>
