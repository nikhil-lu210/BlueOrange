...
@section('page_title', __('All Expenses'))
...
@section('page_name')
    <b class="text-uppercase">{{ __('All Expenses') }}</b>
@endsection
...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item active">{{ __('All Expenses') }}</li>
@endsection
...
<div class="row justify-content-center">
    <div class="col-md-8">
        <form action="{{ route('administration.accounts.income_expense.expense.index') }}" method="get" autocomplete="off">
            <div class="card mb-4">
                <div class="card-body">
                    <div class="row">
                        ...
                        <div class="mb-3 col-md-4">
                            <label class="form-label">{{ __('Expenses Of') }}</label>
                    ...
                    <div class="col-md-12 text-end">
                        @if (request()->category_id || request()->for_month) 
                            <a href="{{ route('administration.accounts.income_expense.expense.index') }}" class="btn btn-danger confirm-warning">
                                <span class="tf-icon ti ti-refresh ti-xs me-1"></span>
                                {{ __('Reset Filters') }}
                            </a>
                        @endif
                        <button type="submit" name="filter_expenses" value="true" class="btn btn-primary">
...
@include('administration.accounts.income_expense.expense.partials._expense_stats')
                        ...
                        <tbody>
                            @foreach ($expenses as $key => $expense) 
                                <tr>
                                    ...
                                    <td>
                                        <b title="Income Date">{{ show_date($expense->date) }}</b>
                                        <br>
                                        <small class="text-muted" title="Entry Date ({{ show_date_time($expense->created_at) }})">{{ date_time_ago($expense->created_at) }}</small>
                                    </td>
                                    <td class="text-center">
                                        @can ('Expense Delete') 
                                            <a href="{{ route('administration.accounts.income_expense.expense.destroy', ['expense' => $expense]) }}" class="btn btn-sm btn-icon btn-danger confirm-danger" data-bs-toggle="tooltip" title="Delete Expense?">
                                                <i class="text-white ti ti-trash"></i>
                                            </a>
                                        @endcan
                                        @can ('Expense Read') 
                                            <a href="{{ route('administration.accounts.income_expense.expense.show', ['expense' => $expense]) }}" class="btn btn-sm btn-icon btn-primary" data-bs-toggle="tooltip" title="Show Details">
