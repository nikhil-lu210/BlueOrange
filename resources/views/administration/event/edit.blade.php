...
@section('page_title', __('Edit Event'))
...
@section('page_name')
    <b class="text-uppercase">{{ __('Edit Event') }}</b>
@endsection
...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item"><a href="{{ route('administration.event.show', ['event' => $event]) }}">{{ $event->title }}</a></li>
...
<div class="row justify-content-center">
    <div class="col-md-10">
        <div class="card">
            ...
            <div class="card-body">
                <form action="{{ route('administration.event.update', ['event' => $event]) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="row">
                        ...
                        <div class="mb-3 col-md-6">
                            ...
                            <select name="event_type" id="event_type" class="form-select @error('event_type') is-invalid @enderror" required>
                                ...
                                <option value="celebration" {{ old('event_type', $event->event_type) == 'celebration' ? 'selected' : '' }}>Celebration</option>
                        ...
                        <div class="mb-3 col-md-6">
                            ...
                            <div class="color-picker-container">
                                ...
                                <div class="color-preview" id="color-preview" style="background-color: {{ old('color', $event->color) }}"></div>
                            </div>
                            @error('color')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="mb-3 col-md-6">
                            <div class="form-check">
                                <input type="checkbox" name="is_all_day" id="is_all_day" value="1" 
                                       class="form-check-input" {{ old('is_all_day', $event->is_all_day) ? 'checked' : '' }}>
                        ...
                        <div class="mb-3 col-md-6">
                            <div class="form-check">
                                <input type="checkbox" name="is_public" id="is_public" value="1" 
                                       class="form-check-input" {{ old('is_public', $event->is_public) ? 'checked' : '' }}>
                        ...
                        <div class="mb-3 col-md-6">
                            ...
                            <div class="row">
                                <div class="col-8">
                                    <input type="number" name="reminder_before" id="reminder_before" value="{{ old('reminder_before', $event->reminder_before) }}" 
                                ...
                                <div class="col-4">
                                    <select name="reminder_unit" id="reminder_unit" class="form-select">
                                        <option value="minutes" {{ old('reminder_unit', $event->reminder_unit) == 'minutes' ? 'selected' : '' }}>Minutes</option>
                                        <option value="hours" {{ old('reminder_unit', $event->reminder_unit) == 'hours' ? 'selected' : '' }}>Hours</option>
                                        <option value="days" {{ old('reminder_unit', $event->reminder_unit) == 'days' ? 'selected' : '' }}>Days</option>
                                    </select>
                                </div>
                            </div>
                        ...
                        <div class="mb-3 col-md-12">
                            ...
                            <select name="participants[]" id="participants" class="select2 form-select" multiple data-allow-clear="true">
                                @foreach ($roles as $role)
                                    <optgroup label="{{ $role->name }}">
                                        @foreach ($role->users as $user)
                                            @php
                                                $isSelected = in_array($user->id, old('participants', $event->participants->pluck('user_id')->toArray()));
                                            @endphp
                    ...
                    <div class="text-end">
                        <a href="{{ route('administration.event.show', ['event' => $event]) }}" class="btn btn-secondary me-2">
