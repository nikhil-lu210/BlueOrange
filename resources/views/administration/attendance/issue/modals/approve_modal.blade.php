        ...
        <div class="modal-content p-3">
            ...
            <div class="modal-body">
                <div class="text-center mb-4">
                    <h3 class="role-title mb-2">{{ ___('Approve Attendance Issue') }}</h3>
                    <p class="text-muted">{{ ___('Approve the Attendance Issue of') }} <b class="text-primary">{{ $issue->user->alias_name }}</b></p>
                </div>
                ...
                <form method="post" action="{{ route('administration.attendance.issue.update', ['issue' => $issue, 'status' => 'Approved']) }}" enctype="multipart/form-data" class="row g-3" autocomplete="off">
                    ...
                    <div class="mb-3 col-md-4">
                        ...
                        <select name="type" id="type" class="form-select bootstrap-select w-100 @error('type') is-invalid @enderror"  data-style="btn-default" required>
                            ...
                            <option value="Regular" {{ old('type', $issue->type) == 'Regular' ? 'selected' : '' }}>{{ ___('Regular') }}</option>
                            <option value="Overtime" {{ old('type', $issue->type) == 'Overtime' ? 'selected' : '' }}>{{ ___('Overtime') }}</option>
                        </select>
                        @error('type')
                            <b class="text-danger"><i class="feather icon-info mr-1"></i>{{ $message }}</b>
                        @enderror
                        ...
                        @if(!$issue->attendance_id)
                            <div id="regularWarning" class="alert alert-warning mt-2" style="display: none;">
                                ...
                                <strong>{{ ___('Warning') }}:</strong> {{ ___('Selecting "Regular" will create a new attendance record. If a Regular attendance already exists for this date, the approval will fail.') }}
                            </div>
                        @endif
                    </div>
                    <div class="col-md-12 mb-3">
                        <label class="form-label">{{ ___('Note') }}</label>
                    ...
                    <div class="col-12 text-center mt-4">
                        <button type="reset" class="btn btn-label-secondary" data-bs-dismiss="modal" aria-label="Close">{{ ___('Cancel') }}</button>
